...
class QuickCommandDialog extends ModalDialog.ModalDialog {
    ...
    _buildUi() {
        ...
        footer.add_child(actionHintBox);
        this._deleteHint =
            this._createFooterHint('Ctrl+Del', this._('Delete'))[0];
        footer.add_child(this._deleteHint);
        footer.add_child(this._createFooterHint('Tab', this._('Switch'))[0]);
        footer.add_child(this._createFooterHint('↑↓', this._('Select'))[0]);
        footer.add_child(this._createFooterHint('Esc', this._('Close'))[0]);
    ...
    _createFooterHint(key, label) {
        const box = new St.BoxLayout({style_class: 'quick-command-footer-hint'});
    ...
    _syncBlur() {
        ...
        if (enabled && !this._blurEffect) {
            ...
            this._panel.add_effect_with_name(
                'quick-command-blur',
                this._blurEffect
            );
        } else if (!enabled && this._blurEffect) {
            this._panel.remove_effect_by_name('quick-command-blur');
            this._blurEffect = null;
        }
    }
