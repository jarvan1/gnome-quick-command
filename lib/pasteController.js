...
export class PasteController {
    ...
    paste(content, targetWindow, useTerminalShortcut = false) {
        if (this._sourceId)
        ...
        this._sourceId = GLib.timeout_add(
            ...
            () => {
                this._sourceId = 0;
                this._deliver(content, targetWindow, useTerminalShortcut);
                return GLib.SOURCE_REMOVE;
            }
        );
    }

    _deliver(content, targetWindow, useTerminalShortcut) {
        ...
        if (typeof content === 'string' && Main.inputMethod.currentFocus) {
            try {
                Main.inputMethod.commit(content);
                return;
            } catch (error) {
    ...
    _sendPasteShortcut(useTerminalShortcut) {
        ...
        try {
            ...
            console.error(`Quick Command could not send paste shortcut: ${error}`);
